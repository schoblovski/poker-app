...
def main():
    ...
    # ── SCHRITT 2: Spieler-Mapping ────────────────────
    print("\n📋 Schritt 2: Spieler aktualisieren...")
    ...
    for row in excel_spieler:
        name, profilbild, eintritt, email, aktiv = row
        updates = {}
        if profilbild: updates['profilbild']     = str(profilbild)
        ...
        if name not in spieler_map:
            new = db.table('spieler').insert({
                ...
                'profilbild':     str(profilbild) if profilbild else None,
    ...
    print(f"  ✅ Spieler-Mapping: {len(spieler_map)} Einträge")
    ...
    # ── SCHRITT 5: Hand Statistik ─────────────────────
    print("\n📋 Schritt 5: Hand Statistik importieren...")
    ...
    for row in hand_rows:
        row = list(row) + [None] * 6
        key, datum, gewinner, hand, kommentar, beweisfoto = row[:6]
        ...
        hand_batch.append({
            ...
            'hand':        str(hand),
            'kommentar':   str(kommentar)  if kommentar  else None,
            'beweisfoto':  str(beweisfoto) if beweisfoto else None,
        })
