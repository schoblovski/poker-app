...
- ~~**Android: natives Chrome-Pull-to-Refresh unterdrückt**~~ ✅ v4.23 – Nur `index.html` (PTR-JS), **keine** DB-/Edge-Function-/Finanzlogik-Änderung, iOS unverändert. **Problem:** Trotz der v4.22-Halte-Geste löste PTR auf Android weiter „extrem sensibel" aus. Ursache: Die Custom-PTR-`touchmove` war `passive:true` und konnte das **native** Chrome-Android-Pull-to-Refresh nicht `preventDefault`en; `overscroll-behavior-y:contain` am `body` griff im body/window-Scroll-Layout nicht zuverlässig. Das native Neuladen feuerte damit zusätzlich zur Custom-PTR, machte einen sofortigen Voll-Reload und ignorierte die 2s-Halte-Geste komplett. **Fix:** `touchmove`-Listener non-passive **nur auf Nicht-iOS** (`{passive:_iosLike}`) und `preventDefault()` genau dann, wenn wir am obersten Seitenrand aktiv nach unten ziehen (`_active && dy>0 && _scrollTop()===0`, gegated durch die bestehende Guard-Kette). Normales Scrollen bleibt unberührt (bei `dy<=0` oder `_scrollTop()>0` wird vor dem `preventDefault` ausgestiegen). **iOS bleibt 100 % unverändert:** dort ist der Listener weiter passiv (`_iosLike===true`) und `preventDefault` wird nie aufgerufen (`!_iosLike`-Guard) → keine Verhaltens-/Scroll-Änderung; iOS-Standalone-PWA hat ohnehin kein natives PTR. Version/Changelog gebumpt (4.22 → 4.23).
- ~~**Android: Pull-to-Refresh entschärft (Halte-Geste)**~~ ✅ v4.22 – Nur `index.html` (CSS + HTML-Markup + PTR-JS), **keine** DB-/Edge-Function-/Finanzlogik-Änderung. **Problem:** PTR auf der Home-Übersicht löste auf Android extrem leicht aus, iOS war unauffällig. **Zwei Ursachen behoben:** (1) Die Scroll-Sperre prüfte `content.scrollTop`, aber in dieser App scrollt der `body`/`window`, nicht `.content` (content wächst auf volle Höhe → `content.scrollTop` blieb auf Android stets 0). Die Sperre „nur oben auslösen" griff dort nie. Neu: Helfer `_scrollTop()=Math.max(content.scrollTop, window.scrollY, documentElement.scrollTop, body.scrollTop)` deckt jeden Scroll-Container ab; Settle-Listener zusätzlich auf `window`. (2) Neue **Halte-Geste**: nach dem Überschreiten der Zug-Schwelle (72 px) muss die Position `HOLD_MS` (2000 ms) gehalten werden, bevor der Reload beim Loslassen startet. Ein SVG-Fortschrittsring (`#ptr-ring`, stroke-basiert, kein Gradient) um das Pill füllt sich via `requestAnimationFrame` (läuft unabhängig von touchmove weiter, auch bei still gehaltenem Finger); bei Voll wird das Symbol grün (`.ready`). Fällt der Zug unter die Schwelle → Halte-Zähler/Ring zurück (kontinuierliches Halten nötig). Neuer `touchcancel`-Handler federt den Balken sauber zurück. **iOS bleibt unverändert:** `_iosLike` (UA `iPad|iPhone|iPod`) setzt `HOLD_MS=0` → sofort scharf an der Schwelle wie bisher, kein Ring (leerer Ring = unsichtbar). Version/Changelog gebumpt (4.21 → 4.22).
